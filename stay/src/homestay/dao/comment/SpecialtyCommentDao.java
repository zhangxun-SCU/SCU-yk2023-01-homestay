...
public class SpecialtyCommentDao {
    DB db = null;

    private void showDebug(String function, String message) {
        String log = "[SpecialtyCommentDao]" + function + message;
    ...
    public void updateComment(Data data, JSONObject json) throws JSONException {
        ...
        try {
            ...
            String sql;
            // 有则更新，无则插入
            if (hasComment(specialty_id, user_id)) {
                sql = String.format(
                        "Update s_order_comment Set score=%d, comment='%s', create_time='%s' Where specialty_id='%s' And user_id='%s'",
                        rating,
                        description,
                        create_time,
                        specialty_id,
                ...
                sql = String.format(
                        "Insert Into s_order_comment Values('%s', '%s', %d, '%s', '%s')",
                        specialty_id,
                        user_id,
                        rating,
                        description,
    ...
    private boolean hasComment(String specialty_id, String user_id) throws SQLException {
        String sql = String.format(
                "Select * From s_order_comment Where specialty_id='%s' And user_id='%s'",
                specialty_id,
                user_id
        );
    ...
    public void getCommentById(String specialty_id, String user_id, JSONObject json) throws JSONException, SQLException {
        String resCode = "00000";
        String info = "success";
        db = new DB("group1");
        try {
            String sql = String.format(
                    "Select score, comment From s_order_comment Where specialty_id='%s' And user_id='%s'",
                    specialty_id,
                    user_id
            );
            ResultSet res = db.executeQuery(sql);
    ...
    public void getComments(String specialty_id, JSONObject json) {
        String resCode = "00000";
        String info = "success";
        db = new DB("group1");
        try {
            String sql = String.format(
                    "Select user_account.avatar, user_account.user_id, score, comment, create_time " +
                            "From s_order_comment Join user_account On s_order_comment.user_id=user_account.user_id " +
            ...
            while (res.next()) {
                ...
                for (int i = 0; i < fieldCount; i++) {
                    ...
                    showDebug("[getComments]", key + ": " + value);
                    map.put(key, value);
                }
                comments.add(map);
            }

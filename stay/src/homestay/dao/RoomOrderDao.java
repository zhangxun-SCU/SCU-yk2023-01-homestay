...
public class RoomOrderDao {
    ...
    public void payRoomOrderRecord(Data data, JSONObject json) throws JSONException, SQLException{
        ...
        int status=1;
        if(order_id!=null){
            String sql="update specialty_order set order_status="+status+" where order_id='"+order_id+"'";
            data.getParam().put("sql",sql);
            updateRecord(data,json);
            showDebug("[payRoomOrder]构造的SQL语句是：" + sql);
        }
    }
    /*修改记录*/
    ...
    private String createGetRecordSql(Data data) throws JSONException {
        ...
        String sql="select * from house, room, room_occupy where room.house_id=room_occupy.house_id and room.house_id=room_occupy.house_id and room_occupy.house_id=house.house_id=specialty.specialty_id and buyer_id='"+username+"'";
        String order_id=data.getParam().has("order_id")?data.getParam().getString("order_id"):null;

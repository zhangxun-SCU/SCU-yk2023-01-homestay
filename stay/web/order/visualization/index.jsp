...
<script type="text/javascript">
    ...
    function getNewOrders() {
        ...
        var data = {"action": "get_new_orders"};
            ...
            if (json.result_code == 0) {
                ...
                if (list != undefined && list.length > 0) {
                    ...
                    for (let i = 0; i < list.length; i++) {
                        let order = list[i];
                        html += `<li>`
                        html += `    <p>\${i+1}&nbsp;<i class="text-white">\${order.buyer_id}</i> 预定了"\${order.house_name}"\${order.room_name}"\${order.num}间 <span class="text-orange">&yen;\${order.price}</span></p>`
                        html += `</li>`
                    }
                    html += "</ul>"
        ...
        // let url = "../../homestay_servlet_room_order_servlet_action";
        // let data = {
    ...
    function getTotalStatistics() {
        ...
        var url = "../../homestay_servlet_room_order_servlet_action";
        var data = {"action": "get_total_statistics"};
        ...
        $.post(url, data, function (json) {
            ...
            if (json.result_code == 0) {
                ...
                if (list != undefined && list.length > 0) {
                    ...
                    for (let i = 0; i < list.length; i++) {
                        let order = list[i];
                        document.getElementById("total_income").innerHTML = order.total_income;
                        document.getElementById("total_sales").innerHTML = order.total_sales;
                    }
                } else {
                    alert("[print_table]与后端交互错误！" + json.result.msg);
                }
            ...
            //     var list = json.aaData;
            //     if (list != undefined && list.length > 0) {
            //         document.getElementById("total_income").innerHTML = list.total_income;
            //         document.getElementById("total_sales").innerHTML = list.total_sales;
            //     } else {
            //         alert("[print_table]与后端交互错误！" + json.result.msg);
            //     }
            // }
